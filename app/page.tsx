    ...
    projects: {
      ...
      items: [
        ...
        },
        {
          ...
          titleFr: "Commande d’un moteur à courant continu avec la modulation de largeur d'impulsions à l’aide d’un hacheur",
          details: "",
        },
      ],
  ...
  de: {
    ...
    projects: {
      ...
      items: [
        ...
        {
          title: "Steuerung eines Gleichstrommotors mittels Pulsweitenmodulation und Chopper",
          titleFr: "Commande d’un moteur à courant continu avec la modulation de largeur d'impulsions à l’aide d’un hacheur",
          details: "",
        },
      ],
...
export default function Portfolio() {
  ...
  const gradientBg = language === "fr" ? "from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800" : "from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800"
  ...
  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText("imadcherradi15@gmail.com") // Replace with actual email logic if needed
      setCopied(true)
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <nav className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4">
        <div className="glass-panel rounded-full px-6 py-3 flex items-center justify-between w-full max-w-5xl animate-fadeInUp">
          ...
          <div className="flex items-center mr-8">
            <span className="text-xl font-bold tracking-tight text-slate-800">
              Imad <span className={`text-${themeColor}-600`}>Cherradi</span>
            </span>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-1">
            {[
              ...
              { id: "tools", label: language === "fr" ? "Outils" : "Werkzeuge" },
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-medium text-slate-600 hover:text-${themeColor}-600 hover:bg-${themeColor}-50/50 rounded-full transition-all capitalize`}
              >
                {section.label}
              </button>
            ))}
          ...
          <div className="hidden md:flex items-center space-x-4 ml-8">
            ...
            <Button
              ...
              className={`rounded-full text-slate-600 hover:text-${themeColor}-600 hover:bg-${themeColor}-50`}
            >
        ...
        {mobileMenuOpen && (
          <div className="absolute top-20 left-4 right-4 glass-panel rounded-2xl p-4 animate-fadeInUp md:hidden flex flex-col space-y-2">
            <button onClick={() => { scrollToSection("hero"); setMobileMenuOpen(false); }} className={`text-left px-4 py-3 text-slate-700 hover:bg-${themeColor}-50 rounded-xl font-medium`}>{t.nav.home}</button>
            <button onClick={() => { scrollToSection("about"); setMobileMenuOpen(false); }} className={`text-left px-4 py-3 text-slate-700 hover:bg-${themeColor}-50 rounded-xl font-medium`}>{t.nav.about}</button>
            <button onClick={() => { scrollToSection("tools"); setMobileMenuOpen(false); }} className={`text-left px-4 py-3 text-slate-700 hover:bg-${themeColor}-50 rounded-xl font-medium`}>{language === "fr" ? "Outils" : "Werkzeuge"}</button>
            <button onClick={() => { scrollToSection("projects"); setMobileMenuOpen(false); }} className={`text-left px-4 py-3 text-slate-700 hover:bg-${themeColor}-50 rounded-xl font-medium`}>{t.nav.projects}</button>
            <button onClick={() => { scrollToSection("contact"); setMobileMenuOpen(false); }} className={`text-left px-4 py-3 text-slate-700 hover:bg-${themeColor}-50 rounded-xl font-medium`}>{t.nav.contact}</button>
            <button
              onClick={() => { toggleLanguage(); setMobileMenuOpen(false); }}
              className={`text-left px-4 py-3 text-slate-700 hover:bg-${themeColor}-50 rounded-xl font-medium flex items-center`}
            >
              <Globe className="w-4 h-4 mr-2" />
              {language === "fr" ? "DE" : "FR"}
            </button>
      ...
      <section
        id="hero"
        className={`relative min-h-screen flex items-center justify-center overflow-hidden pt-24 bg-gradient-to-br from-slate-50 via-${themeColor}-50/30 to-slate-50`}
      >
          ...
          <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 900" preserveAspectRatio="xMidYMid slice">
            <defs>
              <pattern id="circuit" x="0" y="0" width="100" height="100" patternUnits="userSpaceOnUse">
                <circle cx="50" cy="50" r="2" fill={language === 'fr' ? "#3b82f6" : "#0d9488"} opacity="0.3" />
                <line x1="50" y1="50" x2="100" y2="50" stroke={language === 'fr' ? "#3b82f6" : "#0d9488"} strokeWidth="0.5" opacity="0.2" />
                <line x1="50" y1="50" x2="50" y2="0" stroke={language === 'fr' ? "#3b82f6" : "#0d9488"} strokeWidth="0.5" opacity="0.2" />
                {/* Micro-animations in pattern */}
                <circle cx="50" cy="50" r="1" fill={language === 'fr' ? "#60a5fa" : "#2dd4bf"} className="animate-pulse" style={{ animationDuration: '3s' }} />
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#circuit)" className="opacity-20" />

            {/* Dynamic Electric Lines with Glow */}
            <g style={{ filter: `drop-shadow(0 0 5px ${language === 'fr' ? 'rgba(59, 130, 246, 0.5)' : 'rgba(13, 148, 136, 0.5)'})` }}>
              <path d="M0,100 Q400,250 800,100 T1600,100" fill="none" stroke={language === 'fr' ? "#60a5fa" : "#2dd4bf"} strokeWidth="2" strokeDasharray="3000" strokeDashoffset="3000" className="animate-draw opacity-80" />
              <path d="M0,800 Q300,600 600,800 T1600,800" fill="none" stroke={language === 'fr' ? "#3b82f6" : "#0d9488"} strokeWidth="2" strokeDasharray="3000" strokeDashoffset="3000" className="animate-draw opacity-60" style={{ animationDuration: '7s', animationDelay: '1s' }} />
              <path d="M-100,400 Q400,200 900,400 T1900,400" fill="none" stroke={language === 'fr' ? "#22d3ee" : "#34d399"} strokeWidth="1.5" strokeDasharray="3000" strokeDashoffset="3000" className="animate-draw opacity-60" style={{ animationDuration: '10s', animationDelay: '2s' }} />
            </g>
          </svg>
          <div className={`absolute top-20 right-20 w-96 h-96 bg-${themeColor}-400/10 rounded-full blur-3xl animate-pulse-glow`}></div>
          <div className={`absolute bottom-20 left-20 w-72 h-72 bg-${language === 'fr' ? 'cyan' : 'emerald'}-400/10 rounded-full blur-3xl animate-pulse-glow`} style={{ animationDelay: '1.5s' }}></div>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="text-left animate-fadeInUp">
              <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold mb-6 text-slate-900 tracking-tight leading-tight">
                ...
                <br />
                <span className={`text-transparent bg-clip-text bg-gradient-to-r ${gradientText} neon-glow`}>{t.hero.name.split(' ')[1]}</span>
              </h1>
              ...
              <div className="flex lg:hidden justify-center mb-10 animate-float relative">
                {/* Rotating Tech Rings */}
                <div className={`absolute inset-0 border-2 border-${themeColor}-400/20 rounded-full scale-125 opacity-40 border-dashed animate-[spin_20s_linear_infinite]`}></div>
                <div className={`absolute inset-0 border-2 border-${language === 'fr' ? 'cyan' : 'emerald'}-400/20 rounded-full scale-110 opacity-60 border-dashed animate-[spin_15s_linear_reverse_infinite]`}></div>

                <div className={`relative w-64 h-64 sm:w-80 sm:h-80 rounded-full holographic-border p-1 shadow-2xl shadow-${themeColor}-500/20`}>
                  ...
                  {/* Floating Tech Badge */}
                  <div className={`absolute -bottom-6 -right-6 glass-panel px-6 py-4 rounded-2xl flex items-center gap-3 shadow-xl border-2 border-${themeColor}-400/30`}>
                    <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${language === 'fr' ? 'from-blue-500 to-cyan-500' : 'from-teal-500 to-emerald-500'} flex items-center justify-center animate-pulse`}>
                      <Zap className="w-6 h-6 text-white" />
                    </div>
                    <div className="text-sm">
                      <p className="font-bold text-slate-900">Innovation</p>
                      <p className={`text-${themeColor}-600 font-semibold`}>Focused</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-12">
                <Button
                  ...
                  className={`group bg-gradient-to-r ${gradientBg} text-white px-8 py-6 text-lg rounded-xl shadow-lg shadow-${themeColor}-600/30 hover:shadow-${themeColor}-600/50 transition-all font-medium relative overflow-hidden`}
                >
                  <span className={`absolute inset-0 bg-gradient-to-r ${language === 'fr' ? 'from-cyan-400/20 to-blue-400/20' : 'from-emerald-400/20 to-teal-400/20'} translate-y-full group-hover:translate-y-0 transition-transform`}></span>
                ...
                <Button
                  ...
                  className={`holographic-border text-slate-700 hover:text-${themeColor}-600 px-8 py-6 text-lg rounded-xl transition-all font-medium`}
                >
            ...
            <div className="hidden lg:flex justify-center lg:justify-end animate-float relative">
              ...
              <div className={`relative w-64 h-64 sm:w-80 sm:h-80 lg:w-96 lg:h-96 rounded-full holographic-border p-1 shadow-2xl shadow-${themeColor}-500/20`}>
                <div className="w-full h-full rounded-full overflow-hidden relative bg-gradient-to-br from-slate-100 to-slate-200">
                  <Image
                    src="/1718251726199.jpeg"
                    alt="Imad Cherradi - Professional Photo"
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
          <ChevronDown className={`w-6 h-6 text-${themeColor}-400`} />
        </div>
      </section>
      ...
      <section id="about" className="py-24 relative overflow-hidden bg-gradient-to-b from-white via-slate-50/50 to-white">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-20">
          <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
            <path d="M0,50 Q400,100 800,50 T1600,50" fill="none" stroke={language === 'fr' ? "#60a5fa" : "#2dd4bf"} strokeWidth="1" strokeDasharray="1000" strokeDashoffset="1000" className="animate-draw" style={{ animationDuration: '15s' }} />
          </svg>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 relative z-10">
          <div className="text-center mb-12 animate-fadeInUp">
            <h2 className="text-4xl md:text-5xl font-bold mb-6">
              ...
              <span className={`text-transparent bg-clip-text bg-gradient-to-r ${gradientText}`}>{t.about.title.split(' ').slice(1).join(' ')}</span>
            </h2>
          ...
          <div className="holographic-border rounded-2xl shadow-2xl overflow-hidden mb-16 animate-fadeInUp relative bg-white">
            {/* Animated Top Bar */}
            <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${language === 'fr' ? 'from-blue-500 via-cyan-400 to-purple-500' : 'from-teal-500 via-emerald-400 to-cyan-500'} animate-holographic`}></div>
            ...
            <div className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200 px-4 pt-8 pb-4 md:px-8 md:pt-10 md:pb-6 flex justify-center gap-6 md:gap-16 relative">
              <button
                ...
                className={`group pb-2 md:pb-4 text-lg md:text-2xl font-bold tracking-wide transition-all flex items-center gap-2 md:gap-4 relative ${activeTab === 'education' ? `text-${themeColor}-600` : 'text-slate-400 hover:text-slate-600'}`}
              >
                <GraduationCap className="w-7 h-7" />
                <span>{t.about.education}</span>
                {/* Animated underline */}
                <div className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r ${language === 'fr' ? 'from-blue-500 to-cyan-500' : 'from-teal-500 to-emerald-500'} transition-all ${activeTab === 'education' ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0'}`}></div>
              </button>

              <button
                onClick={() => setActiveTab('experience')}
                className={`group pb-2 md:pb-4 text-lg md:text-2xl font-bold tracking-wide transition-all flex items-center gap-2 md:gap-4 relative ${activeTab === 'experience' ? `text-${themeColor}-600` : 'text-slate-400 hover:text-slate-600'}`}
              >
            ...
            <div className={`p-8 min-h-[400px] bg-gradient-to-br from-white via-slate-50/30 to-${themeColor}-50/20 relative`}>
              {/* Decorative circuit lines */}
              <div className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-10">
                <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
                  <line x1="0" y1="50%" x2="100%" y2="50%" stroke={language === 'fr' ? "#3b82f6" : "#0d9488"} strokeWidth="1" strokeDasharray="5,5" opacity="0.3" />
                  <circle cx="10%" cy="50%" r="3" fill={language === 'fr' ? "#3b82f6" : "#0d9488"} />
                  <circle cx="90%" cy="50%" r="3" fill={language === 'fr' ? "#06b6d4" : "#10b981"} />
                </svg>
              </div>
                  ...
                  {t.about.educationItems.map((item, index) => (
                    <div key={index} className={`group relative pl-10 border-l-2 border-${themeColor}-200 hover:border-${themeColor}-500 transition-all pb-2`}>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1 mb-2">
                        <h4 className={`font-bold font-sans text-slate-900 text-lg group-hover:text-${themeColor}-600 transition-colors`}>{item.degree}</h4>
                        <span className={`font-mono text-xs font-bold text-${themeColor}-600 bg-gradient-to-r from-${themeColor}-50 to-${language === 'fr' ? 'cyan' : 'emerald'}-50 px-3 py-1.5 rounded-full whitespace-nowrap border border-${themeColor}-200 shadow-sm`}>
                          <span className={`inline-block w-1.5 h-1.5 rounded-full bg-${themeColor}-500 mr-2 animate-pulse`}></span>
                          {item.date}
                        </span>
                      </div>
        ...
        <div className="max-w-6xl mx-auto px-4 relative z-10">
          ...
          <div className="text-center mb-16 animate-fadeInUp">
            <h2 className="text-4xl md:text-5xl font-bold mb-6">
              ...
              <span className={`text-transparent bg-clip-text bg-gradient-to-r ${gradientText}`}>{t.projects.title.split(' ').slice(1).join(' ')}</span>
            </h2>
            <p className="text-xl text-slate-600 max-w-2xl mx-auto font-light">{t.projects.subtitle}</p>

            {/* Project Counter */}
            <div className={`mt-6 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-${themeColor}-50 to-${language === 'fr' ? 'cyan' : 'emerald'}-50 border border-${themeColor}-200`}>
              <div className={`w-2 h-2 rounded-full bg-${themeColor}-500 animate-pulse`}></div>
              <span className={`font-mono text-sm text-${themeColor}-600 font-bold`}>{projects.length} Projects</span>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((proj, index) => (
              ...
              >
                <div className={`holographic-border rounded-2xl overflow-hidden bg-white shadow-lg hover:shadow-2xl hover:shadow-${themeColor}-500/20 transition-all duration-500 h-[420px] relative`}>
                  ...
                  <div className="p-6 flex flex-col justify-between h-[164px] relative">
                    {/* Decorative line */}
                    <div className={`absolute top-0 left-6 right-6 h-px bg-gradient-to-r from-transparent via-${themeColor}-200 to-transparent`}></div>

                    <div>
                      <h3 className={`text-xl font-bold text-slate-900 group-hover:text-${themeColor}-600 transition-colors duration-300 line-clamp-2 mb-2`}>
                        {proj.title}
                      </h3>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                      ...
                      {/* Animated arrow */}
                      <div className={`w-8 h-8 rounded-full bg-slate-50 group-hover:bg-${themeColor}-50 flex items-center justify-center transition-all duration-300`}>
                        <ChevronDown className={`w-4 h-4 text-slate-400 group-hover:text-${themeColor}-600 -rotate-90 transition-all duration-300 group-hover:translate-x-1`} />
                      </div>
                    </div>
                  </div>

                  {/* Hover glow effect */}
                  <div className={`absolute inset-0 bg-gradient-to-t from-${themeColor}-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none`}></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="contact" className="py-24 bg-gradient-to-b from-white to-slate-50 border-t border-slate-200/60 relative overflow-hidden">
        ...
        <div className={`absolute top-0 right-0 w-[500px] h-[500px] bg-${themeColor}-100/40 rounded-full blur-3xl opacity-50 translate-x-1/2 -translate-y-1/2 pointer-events-none`}></div>
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-12 mb-24">
            ...
            <div className="space-y-6 max-w-sm animate-fadeInUp">
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900 leading-tight">
                {t.contact.headerTitle} <span className={`text-transparent bg-clip-text bg-gradient-to-r ${gradientText}`}>{t.contact.headerAccent}</span>{t.contact.headerTitleEnd}
              </h2>
            ...
            <div className="flex flex-col gap-8 w-full md:min-w-[300px] animate-fadeInUp" style={{ animationDelay: '0.1s' }}>
              ...
              <div className="space-y-3">
                ...
                <div
                  ...
                  className={`holographic-border group cursor-pointer relative bg-white rounded-xl p-4 flex items-center justify-between gap-4 transition-all duration-300 hover:shadow-lg hover:shadow-${themeColor}-500/10 border border-slate-200`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`p-2.5 rounded-lg bg-${themeColor}-50 text-${themeColor}-600 group-hover:bg-${themeColor}-600 group-hover:text-white transition-colors duration-300`}>
                      <Mail className="w-5 h-5" />
                    </div>
                    <span className={`font-medium text-slate-700 group-hover:text-${themeColor}-700 transition-colors`}>{t.contact.info.emailValue}</span>
                  </div>

                  <div className={`flex-shrink-0 text-slate-400 group-hover:text-${themeColor}-600 transition-colors`}>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <a
                    ...
                    className={`flex items-center gap-3 p-4 rounded-xl bg-white border border-slate-200 hover:border-${themeColor}-400 hover:shadow-md transition-all group`}
                  >
          ...
          <div className="pt-8 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-mono text-slate-400 uppercase tracking-wider">
            ...
            <div className="flex gap-6">
              <span className={`hover:text-${themeColor}-600 transition-colors cursor-default flex items-center gap-2`}>
                <Globe className="w-3 h-3" />
                {t.footer.timezone}: GMT+1
              </span>
              <span className={`hover:text-${themeColor}-600 transition-colors cursor-default`}>
